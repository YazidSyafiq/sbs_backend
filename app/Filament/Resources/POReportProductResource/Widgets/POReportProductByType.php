...
class POReportProductByType extends BaseWidget
{
    protected function getStats(): array
    {
        ...
        $statsArray = [
            ...
            Stat::make($isUserRole ? 'Credit Paid' : 'Credit Received', 'Rp ' . number_format($stats->credit_paid_amount, 0, ',', '.'))
                ->description($stats->credit_payment_rate . '% paid')
                ->descriptionIcon('heroicon-m-banknotes')
                ->color('success'),

            Stat::make($isUserRole ? 'Credit Unpaid' : 'Credit Outstanding', 'Rp ' . number_format($stats->credit_outstanding, 0, ',', '.'))
            ...
            Stat::make($isUserRole ? 'Cash Paid' : 'Cash Received', 'Rp ' . number_format($stats->cash_paid_amount, 0, ',', '.'))
                ->description($stats->cash_payment_rate . '% paid')
                ->descriptionIcon('heroicon-m-currency-dollar')
                ->color('success'),

            Stat::make($isUserRole ? 'Cash Unpaid' : 'Cash Outstanding', 'Rp ' . number_format($stats->cash_outstanding, 0, ',', '.'))
