...
class POReportServiceProfitByBranch extends BaseWidget
{
    protected function getStats(): array
    {
        $filters = session('po_service_filters', []);
        $branchProfits = POReportService::getFilteredProfitByBranch($filters);
    ...
    public function getHeading(): ?string
    {
        ...
        $title = 'Service Profit Analysis by Branch';
        if ($totalBranches > 8) {
    ...
    {
        return 'Realized profit from paid service orders vs potential profit from unpaid orders';
    }
}
