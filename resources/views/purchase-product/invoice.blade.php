...
<head>
    <meta charset="utf-8">
    <title>Invoice - {{ $purchaseProduct->po_number }}</title>
...
<body>
    ...
    @if (!$isFromMobile)
        <!-- Action Buttons - Only show if NOT from mobile -->
                    ...
                    <td style="width: 50%; vertical-align: top; padding-left: 10px;">
                        <table class="po-details-table">
                            ...
                            <tr>
                                ...
                                <td class="po-info-value">: {{ ucfirst($purchaseProduct->type_po) }}</td>
                            </tr>
                            <tr>
        ...
        <div class="table-container">
            ...
            <table class="items-table">
                ...
                <tbody>
                    ...
                    @endforeach
                    <tr class="total-row">
                        <td colspan="4" style="text-align: right;"><strong>TOTAL PO :</strong></td>
        ...
        <table class="compact-row" style="width: 100%; margin-top: 8px;">
            <tr>
                ...
                <td class="compact-cell" style="width: 40%; padding-right: 6px;">
                    ...
                    @if ($purchaseProduct->status_paid || $purchaseProduct->bukti_tf)
                        <div class="payment-info">
                            <div class="payment-title">Info Pembayaran</div>
                            @if ($purchaseProduct->status_paid)
                                <p style="margin: 2px 0; font-size: 7px;">Status:
                                    <span class="payment-status-{{ $purchaseProduct->status_paid }}">
                                        {{ $purchaseProduct->status_paid === 'paid' ? 'Lunas' : 'Belum Lunas' }}
                                    </span>
                                </p>
                            @endif
                            @if ($purchaseProduct->bukti_tf)
    ...
    <script>
        ...
        async function downloadInvoicePDF() {
            try {
                ...
                const filename =
                    `Invoice_{{ $purchaseProduct->po_number }}_${new Date().toISOString().slice(0, 19).replace(/:/g, '-')}.pdf`;
