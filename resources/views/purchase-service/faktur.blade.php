...
<head>
    <meta charset="utf-8">
    <title>Faktur Service - {{ $purchaseService->po_number }}</title>
...
<body>
    ...
    <div class="container" id="faktur-content">
        <!-- Header -->
        <div class="header">
            <h1>FAKTUR PURCHASE ORDER SERVICE</h1>
                ...
                <div class="party-section customer-section">
                    ...
                    <table class="party-details-table">
                        <tr>
                            ...
                            <td class="party-info-value">: {{ $purchaseService->user->name }}</td>
                        </tr>
                        @if ($purchaseService->user->branch)
                            <tr>
                                <td class="party-info-label">Cabang</td>
                                <td class="party-info-value">: {{ $purchaseService->user->branch->name }}</td>
                            </tr>
                            <tr>
                                <td class="party-info-label">Kode Cabang</td>
                                <td class="party-info-value">: {{ $purchaseService->user->branch->code }}</td>
                            </tr>
                        @endif
                    </table>
                </div>
            ...
            <div class="details-grid">
                <div>
                    <div class="detail-item">
                        ...
                        <span class="detail-value">: {{ $purchaseService->po_number }}</span>
                    </div>
                    <div class="detail-item">
                        <span class="detail-label">Tanggal Faktur</span>
                        <span class="detail-value">: {{ $purchaseService->order_date->format('d F Y') }}</span>
                    </div>
                    <div class="detail-item">
                        <span class="detail-label">Jenis PO</span>
                        <span class="detail-value">: {{ ucfirst($purchaseService->type_po) }} Purchase</span>
                    </div>
                </div>
                <div>
                    <div class="detail-item">
                        <span class="detail-label">Status Bayar</span>
                        <span class="detail-value">: <span class="payment-status-{{ $purchaseService->status_paid }}">
                                {{ $purchaseService->status_paid === 'paid' ? 'LUNAS' : 'BELUM LUNAS' }}
                            </span>
                        </span>
                    </div>
                    @if ($purchaseService->expected_delivery_date)
                        <div class="detail-item">
                            <span class="detail-label">Target Proccess</span>
                            <span class="detail-value">:
                                {{ $purchaseService->expected_proccess_date->format('d F Y') }}</span>
                        </div>
                    @endif
                    <div class="detail-item">
                        <span class="detail-label">Jumlah Item</span>
                        <span class="detail-value">: {{ $purchaseService->items->count() }} items</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="table-container">
            ...
            <table class="items-table">
                ...
                <tbody>
                    @foreach ($purchaseService->items as $index => $item)
                        <tr>
                            <td style="text-align: center; font-weight: 600; color: #28a745;">{{ $index + 1 }}</td>
                            <td>
                                <strong>{{ $item->service->name }}</strong>
                            </td>
                            <td style="text-align: center;">{{ $item->service->code }}</td>
                            <td style="text-align: center; font-weight: 600;">{{ $item->technician->name }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
        ...
        <div class="signature-section">
            <table class="signature-table">
                <tr>
                    ...
                    <td class="signature-cell">
                        <div class="signature-label">DIPROSES OLEH</div>
                        <div class="signature-box"></div>
                    </td>
                    <td class="signature-cell">
    ...
    <script>
        ...
        async function downloadFakturPDF() {
            try {
                ...
                const filename =
                    `Faktur_{{ $purchaseService->po_number }}_${new Date().toISOString().slice(0, 19).replace(/:/g, '-')}.pdf`;
