...
<head>
    <meta charset="utf-8">
    <title>Service Purchase Report - {{ $fromDate->format('d/m/Y') }} to {{ $untilDate->format('d/m/Y') }}</title>
                        ...
                        @foreach ($reportData as $data)
                            ...
                            @for ($i = 0; $i < $itemsCount; $i++)
                                ...
                                <tr>
                                    ...
                                    <td style="border-right: 1px solid #dee2e6;">{{ $item->service->name }}
                                        ({{ $item->service->code }})</td>
                                    <td style="border-right: 1px solid #dee2e6;">{{ $item->technician->name ?? '-' }}
                                    </td>
                                    ...
                                    @if ($i == 0)
                                        ...
                                        <td rowspan="{{ $itemsCount }}"
                                            style="text-align: center; vertical-align: middle; border-right: 1px solid #dee2e6;">
                                            <span
                                                class="status-badge payment-{{ $purchase->status_paid }}">{{ ucfirst($purchase->status_paid) }}</span>
                                        </td>
                                    @endif

                                    <!-- Financial Information per item -->
                                    ...
                                    <td style="text-align: right; border-right: 1px solid #dee2e6;">Rp
                                        {{ number_format($item->cost_price ?? 0, 0, ',', '.') }}</td>
                                    <td
                                        style="text-align: right; font-weight: bold; color: {{ $item->selling_price - ($item->cost_price ?? 0) >= 0 ? '#28a745' : '#dc3545' }};">
                                        Rp
                                        {{ number_format($item->selling_price - ($item->cost_price ?? 0), 0, ',', '.') }}
                                    </td>
                                </tr>
                            @endfor
                        @endforeach
    ...
    <script>
        async function downloadReportPDF() {
            try {
                ...
                const filename =
                    `Service_Purchase_Report_${new Date().toISOString().slice(0, 19).replace(/:/g, '-')}.pdf`;
                ...
                pdf.setProperties({
                    title: filename,
                    subject: 'Service Purchase Report',
