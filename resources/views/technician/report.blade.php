...
<head>
    <meta charset="utf-8">
    <title>Technician Report - {{ $fromDate->format('d/m/Y') }} to {{ $untilDate->format('d/m/Y') }}</title>
    ...
    <style>
        ...
        .summary-card.average-price {
            border-left: 4px solid #27ae60;
        }

        .summary-card.average-po {
    ...
    <div class="container" id="report-content">
        ...
        <div class="report-info">
            ...
            <div class="info-grid">
                <div>
                    ...
                    <div class="info-item">
                        <span class="info-label">Total Technicians</span>
                        <span class="info-value">: {{ $totalTechnicians }} technicians</span>
                    </div>
                    <div class="info-item">
                        <span class="info-label">Average Fee</span>
                        <span class="info-value">: Rp {{ number_format($averagePrice, 0, ',', '.') }}</span>
                    </div>
                </div>
                <div>
                    <div class="info-item">
                    ...
                    <div class="info-item">
                        <span class="info-label">Clear Status</span>
                        <span class="info-value">: {{ $piutangSummary['no_piutang'] }} technicians</span>
                    </div>
                </div>
            </div>
            ...
            <table class="report-table">
                <thead>
                    <tr>
                        ...
                        <th class="col-name">TECHNICIAN</th>
                        <th class="col-contact">SERVICE</th>
                        <th class="col-address">ORDER DATE</th>
                        <th class="col-price">SERVICE FEE</th>
                        <th class="col-piutang">COST</th>
                        <th class="col-total-po">REVENUE</th>
                        <th class="col-status">PAYMENT STATUS</th>
                    </tr>
                </thead>
                <tbody>
                    @if (count($filteredItems) > 0)
                        @foreach ($filteredItems as $index => $data)
                            <tr>
                                <td class="col-no">{{ $index + 1 }}</td>
                                <td class="col-code">{{ $data->purchase->po_number }}</td>
                                <td class="col-name">
                                    {{ $data->technician->name ?? '-' }}
                                    @if ($data->technician)
                                        <br><small>({{ $data->technician->code }})</small>
                                    @endif
                                </td>
                                <td class="col-contact">
                                    {{ $data->service->name ?? '-' }}
                                    @if ($data->service)
                                        <br><small>({{ $data->service->code }})</small>
                                    @endif
                                </td>
                                <td class="col-address">{{ $data->purchase->order_date->format('d/m/Y') }}</td>
                                <td class="col-price">Rp
                                    {{ number_format($data->technician->price ?? 0, 0, ',', '.') }}</td>
                                <td class="col-piutang">Rp
                                    {{ number_format($data->item->cost_price ?? 0, 0, ',', '.') }}</td>
                                <td class="col-total-po">Rp
                                    {{ number_format($data->item->selling_price ?? 0, 0, ',', '.') }}</td>
                                <td class="col-status">
                                    <span
                                        class="status-badge status-{{ $data->purchase->status_paid === 'paid' ? 'no-piutang' : 'has-piutang' }}">
                                        {{ $data->purchase->status_paid === 'paid' ? 'PAID' : 'UNPAID' }}
                                    </span>
                                </td>
                            </tr>
                        @endforeach
        ...
        <div class="summary-section">
            ...
            <div class="summary-grid">
                <div class="summary-card technicians">
                    <div class="summary-label">Total Technicians</div>
                    <div class="summary-value">{{ $totalTechnicians }} Technicians</div>
                </div>
                ...
                <div class="summary-card average-price">
                    <div class="summary-label">Average Service Fee</div>
                    <div class="summary-value">Rp {{ number_format($averagePrice, 0, ',', '.') }}</div>
                </div>
                <div class="summary-card average-po">
                    <div class="summary-label">Average PO per Technician</div>
    ...
    <script>
        async function downloadReportPDF() {
            try {
                ...
                const filename =
                    `Technician_Report_${new Date().toISOString().slice(0, 19).replace(/:/g, '-')}.pdf`;
